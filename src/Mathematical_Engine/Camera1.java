...
public class Camera1 {
    ...
    public Camera1(int Sx, int Sy, int Ox, int Oy) {
        s2 = new S2(Sx, Sy, Ox, Oy);
        frustum = new Frustum(this, distancetoFrustum, Sx, Sy);
    }
    ...
    public V2 project(V3 p) {
        ...
        // Check if d is close to zero
        if (Math.abs(d) < 1e-6) {
            // Handle this case to prevent division by zero
            return new V2(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
        }

        double u = U.dot(EP);
        double r = R.dot(EP);
        double um = u * z / d;
        double rm = r * z * aspectRatio / d; // Apply aspect ratio correction

        // Calculate the projected point
        V3 projectedPoint = calculateProjectionMatrix().mul(p.sub(E));

        double newX = projectedPoint.x;
        double newY = projectedPoint.y;
    ...
    public void moveTo(V3 p, int screenWidth, int screenHeight) {
        ...
        updateFrustum(screenWidth, screenHeight);
    }

    public void focus(V3 p, int screenWidth, int screenHeight) {
        // Calculate the new direction vector
        D = p.sub(E).unit();
        // Calculate the right vector (R) by crossing D with the world's up vector (j)
        R = D.cross(j).unit();
        // Calculate the new up vector (U) by crossing R with D
        U = R.cross(D);
        updateFrustum(screenWidth, screenHeight);
    }
    ...
    public void moveDirection(Set<String> directions) {
        double moveAmount = 1; // Adjust the amount as needed
    ...
    public void updateFocusPointWithMouse(int mouseX, int mouseY, int screenWidth, int screenHeight, Component component) {
        ...
        pitch(deltaY * sensitivity * sensitivity);
    ...
    public boolean isPointInFrustum(V3 point, int screenWidth, int screenHeight) {
        updateFrustum(screenWidth, screenHeight);
        return frustum.containsPoint(point);
    }

    private void updateFrustum(int screenWidth, int screenHeight) {
        // Calculate the distance to move the frustum along the camera's direction vector

        // Update the frustum with the new camera position
        frustum.update(this, screenWidth, screenHeight);
    }
}
